...
from ..backends import create_backend, create_default_backend, list_backends
from ..core import Translator
from ..core.constants import OPENAI_TTS_DEFAULT_VOICE, OPENAI_TTS_VOICES
...
class ServerState:
    ...
    __slots__ = ("guild_id", "translator", "backend_name", "voice", "tts_mode")

    _TTS_MODES = ("normal", "rude")
    ...
    def __init__(
        ...
        self.tts_mode = self._normalize_tts_mode(tts_mode)
        ...
        logger.info(
            "[server:%d] Initialized (backend=%s, voice=%s)",
            guild_id,
            self.backend_name,
            self.voice,
        )
    ...
    def _normalize_tts_mode(self, mode: Optional[str]) -> str:
        ...
        return m if m in self._TTS_MODES else "normal"
    ...
    def switch_backend(self, name: str) -> str:
        ...
        # Swap backend in-place so any active voice session keeps working
        # (AudioSink/TranslationPipeline keep a reference to the Translator instance).
        self.translator.set_backend(new_backend)
    ...
    def switch_tts_mode(self, mode: str) -> str:
        """Switch TTS playback mode. Returns new mode."""
        m = self._normalize_tts_mode(mode)
        if m != str(mode).strip().lower():
            raise ValueError(f"Unknown TTS mode '{mode}'. Available: {', '.join(self._TTS_MODES)}")
        self.tts_mode = m
        logger.info("[server:%d] Switched to TTS mode=%s", self.guild_id, m)
        return m
...
class ServerStateManager:
    ...
    def get(self, guild_id: int) -> ServerState:
        ...
        if guild_id not in self._states:
            ...
            tts_mode = saved.get("tts_mode")
            self._states[guild_id] = ServerState(guild_id, backend_name, voice, tts_mode)
        return self._states[guild_id]

    def save(self, guild_id: int) -> None:
        """Persist current state for a guild."""
        if guild_id in self._states:
            ...
            logger.debug("[persist] Saved settings for guild %d", guild_id)
